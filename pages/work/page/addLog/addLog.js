...
Page({
  ...
  _bindChooseOutPutTime() {
    console.log(22)
    // onSaveDialogScreenIsOutPutRef
    // if(this.onSaveDialogScreenIsOutPutRef) this.onSaveDialogScreenIsOutPutRef._showDialog();
    if (this.dialogPickerDateRef) this.dialogPickerDateRef._showDialog();
  },
    ...
    request.doPostRequest({
      ...
      success: res => {
        this.setData({
          formData: res.data,
          isAccess: {
            name: this.data.screenFromList.find(e => e.value === res.data.isAccess).name,
            value: res.data.isAccess
          },
          projectClassification: {
            name: res.data.projectClassification_dictText,
            value: res.data.projectClassification
          },
          constructionPhase: {
            name: res.data.constructionPhase_dictText,
            value: res.data.constructionPhase
          },
          isOutPut: {
            name: this.data.isOutPutOption.find(e => e.value === res.data.isOutPut).name,
            value: res.data.isOutPut
          },
          'outPutTime.shortDate': res.data.outPutTime,

          constructionNature: {
            name: res.data.constructionNature_dictText,
            value: res.data.constructionNature
          },
          engineeringProperties: {
            name: res.data.engineeringProperties_dictText,
            value: res.data.engineeringProperties
          },
  ...
  getCodeList() {
    ...
    request.doPostRequest({
      ...
      data: {
        dictCode: 'project_classification'
      },
      success: res => {
        res.data.forEach(e => {
          e.name = e.itemText
    ...
    request.doPostRequest({
      ...
      data: {
        dictCode: 'engineering_properties'
      },
      success: res => {
        res.data.forEach(e => {
          e.name = e.itemText
